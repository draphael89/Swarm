# Terminal/Shell Support Plan for Swarm Chat UI

## Objective
Add terminal support to Swarm’s chat experience so users can:
...
- Recommendation: implement native terminal support in Swarm, while reusing general panel/tabs interaction patterns only.
...
```bash
pnpm --filter @swarm/backend add node-pty
pnpm --filter @swarm/ui add @xterm/xterm @xterm/addon-fit
pnpm --filter @swarm/ui add @xterm/addon-web-links @xterm/addon-search
```
